...
describe.skip('Splitting the pot between two players', function () {
  ...
  it('should give an extra chip to the player on the left of the dealer seat, when there is an odd chip', function () {
    ...
    var playerOnDealersRight = table.findPreviousPlayer(
      table.public.dealerSeat
    );
    console.log(playerOnDealersRight);
    for (var i = 0; i < 2; i++) {
      expect(table.seats[playerOnDealersLeft].public.chipsInPlay).toEqual(
        table.seats[playerOnDealersRight].public.chipsInPlay + 1
      );
    }
  });
});
